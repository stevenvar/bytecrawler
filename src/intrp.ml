...
let interp level bytecode state prims =
...
let interp file =
  ...
  interp 0 bytecode (new_state global) primitives;
  Format.printf "Max heap size = %d\n" !max_heap;
  Format.printf "Allocated %d blocks total in heap\n" !nb_blocks;
  Format.printf "Max size of the stack = %d levels\n" !max_stack
